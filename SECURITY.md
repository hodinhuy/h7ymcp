...
H7Y MCP exposes a local workspace to an MCP client. Treat it like a developer tool with access to your source tree, not like a hosted SaaS app.

Phase 1 migration note:

- canonical CLI: `h7ymcp`
- canonical env namespace: `PERSONAL_*`
- canonical config home: `~/.personal-edition`
- legacy `CODEXPRO_*` and `~/.h7ymcp` are still accepted temporarily for compatibility
...
H7Y MCP is not designed to bypass, avoid, pool, resell, or modify ChatGPT, Codex, OpenAI, or third-party model limits. Do not market, deploy, or configure it that way.
...
- optional local handoff execution through `h7ymcp execute-handoff`, run from the user's terminal only
- optional local execute/review looping through `h7ymcp loop-handoff`, run from the user's terminal only with a user-provided reviewer command and iteration limit
...
| --- | --- |
| Public tunnel reachable without a secret | Public/non-loopback HTTP fails closed unless a H7Y MCP token is configured. |
| Raw H7Y MCP or Cloudflare token appears in UI, logs, docs, or package output | Tokens are redacted in profile/status output and tunnel tokens use local files for persistence. |
...
- leaking a bearer auth token or Cloudflare tunnel token
...
For stable public hostnames, keep the H7Y MCP auth token stable but private:

```bash
h7ymcp start \
  --root /path/to/repo \
  --tunnel cloudflare-named \
  --hostname h7ymcp.example.com \
  --tunnel-name h7ymcp \
...
- Do not commit printed bearer tokens or header examples that include the live auth token.
...
- Preview local handoff execution with `h7ymcp execute-handoff --dry-run` before running an unfamiliar adapter or custom command.
- Preview autonomous local loops with `h7ymcp loop-handoff --dry-run`, keep `--max-iters` small, and prefer `--require-human-confirmation` until you trust the reviewer command.
...
- Use `--bash-session <id> --require-bash-session` when bash should be enabled only for calls that explicitly target this local H7Y MCP terminal label.
...
- Keep `CODEXPRO_CONTEXT_DIR` as a workspace-relative hidden directory such as `.ai-bridge`; H7Y MCP rejects source, build, dependency, credential, and absolute context directories.
- Use `--bash full` only for trusted local repos.
- Do not treat MCP session ids or bash session labels as Codex conversation ids. H7Y MCP does not execute inside a Codex app session.
...
For the one-command public tunnel flow, H7Y MCP can download the official Cloudflare `cloudflared` release into `~/.h7ymcp/bin` on supported macOS, Windows, and Linux systems. It does not install a system service, does not use sudo/admin rights, and does not modify shell startup files.
...
3. ~/.h7ymcp/bin/cloudflared or cloudflared.exe
...
H7Y MCP blocks common sensitive paths by default:
