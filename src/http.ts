...
import {
  BASH_MODES,
  BASH_TRANSCRIPT_MODES,
  CODEX_SESSIONS_MODES,
  TOOL_MODES,
  WRITE_MODES,
  effectiveWriteMode,
  expandHome,
  loadConfig,
  normalizeBashMode,
  normalizeBashTranscriptMode,
  normalizeCodexSessionsMode,
  normalizeToolMode,
  normalizeTunnelMode,
  normalizeWidgetDomain,
  type CodexProConfig
} from "./config.js";
import { EDITION, authorizedUrlDisplay, editionEnvValue } from "./edition.js";
import { DEFAULT_OAUTH_SCOPES, LocalOAuthProvider, LocalOAuthState } from "./oauth.js";
import {
  profilePathForRoot,
...
const BASH_TRANSCRIPTS = BASH_TRANSCRIPT_MODES;
const CODEX_SESSIONS = CODEX_SESSIONS_MODES;
...
function runtimeTunnelFallback(): TunnelMode {
  return normalizeTunnelMode(editionEnvValue("TUNNEL"), "none");
}
...
function normalizeOptionalWidgetDomain(value: string | undefined): string {
  const raw = value?.trim() ?? "";
  return raw ? normalizeWidgetDomain(raw) : "";
}
...
function profileValues(config: CodexProConfig, profile = readWorkspaceProfile(config.defaultRoot)): ProfileFormValues {
  const hostname =
    profile.hostname ??
    editionEnvValue("PUBLIC_HOSTNAME") ??
    editionEnvValue("HOSTNAME") ??
    process.env.NGROK_DOMAIN ??
    "";
  const mode = oneOf(profile.mode ?? editionEnvValue("MODE"), MODES, "agent");
  ...
  return {
    ...
    bash: normalizeBashMode(String(profile.bash ?? config.bashMode)),
    bashTranscript: normalizeBashTranscriptMode(String(profile.bashTranscript ?? config.bashTranscript)),
    codexSessions: normalizeCodexSessionsMode(String(profile.codexSessions ?? config.codexSessions)),
    ...
    write,
    toolMode: normalizeToolMode(String(profile.toolMode ?? config.toolMode)),
...
function serverUrlDisplay(endpoint: string | undefined, authEnabled: boolean): string {
  return authorizedUrlDisplay(endpoint, authEnabled);
}

function connectorAuthLabel(config: CodexProConfig): string {
  if (config.httpAuthMode === "oauth") return "OAuth";
  return config.authToken ? "Token protected" : "Disabled";
}

function chatgptAuthCopy(config: CodexProConfig): string {
  if (config.httpAuthMode === "oauth") {
    return "Choose OAuth. ChatGPT should discover the server's OAuth metadata automatically from this URL.";
  }
  if (config.authToken) {
    return "Use the copied URL with a Bearer header if your client supports custom headers. For trusted local-only runs, you can also restart without auth.";
  }
  return "Use No Authentication / None.";
}

function requestOrigin(req: Request): URL {
  ...
  if (!hostHeader || typeof hostHeader !== "string") {
    throw new Error("Missing Host header for OAuth discovery.");
  }
  return new URL(`${proto}://${hostHeader}`);
}

function isOAuthPublicPath(reqPath: string): boolean {
  return (
    reqPath === "/authorize" ||
    reqPath === "/token" ||
    reqPath === "/register" ||
    reqPath === "/revoke" ||
    reqPath === "/oauth/approve" ||
    reqPath === "/oauth/deny" ||
...
function currentTunnelMessage(tunnel: TunnelMode, endpoint: string): string {
  ...
  if (tunnel === "ngrok") return "Enter your reserved ngrok domain, or set NGROK_DOMAIN before starting the personal edition.";
...
function profileForm(config: CodexProConfig): string {
  ...
  return `<section class="panel profile-panel" id="profile">
      ...
      <form class="profile-form" data-profile-form>
        ...
        <p class="note" data-profile-status>Tokens stay hidden. Restart ${EDITION.productName} for saved profile changes to apply.</p>
      </form>
    </section>`;
}
...
function buildProfilePayload(config: CodexProConfig, existing: WorkspaceProfile, input: AdminProfilePatch): WorkspaceProfile {
  ...
  next.widgetDomain = normalizeOptionalWidgetDomain(next.widgetDomain);
...
function profileResponse(config: CodexProConfig): Record<string, unknown> {
  ...
  return {
    ...
    runtime: {
      ...
      httpAuthMode: config.httpAuthMode
    }
  };
}
...
function onboardingPage(config: CodexProConfig): string {
  ...
  const localMcpDisplay = authorizedUrlDisplay(localMcp, Boolean(config.authToken));
  const allowedRoots = config.allowedRoots.map((root) => `<li>${escapeHtml(root)}</li>`).join("");
  const authModeLabel = connectorAuthLabel(config);
  ...
  const githubUrl = EDITION.repositoryUrl;
  const npmUrl = EDITION.homepageUrl;
  const docsUrl = EDITION.docsUrl;
  const chatgptUrl = EDITION.connectorSettingsUrl;
  const controls = [
    copyCommand("Re-run setup wizard", "Use the CLI for broader profile edits that are intentionally not exposed here.", `${EDITION.cliName} setup`),
    ...
    copyCommand("Start without bash", "Restart with file tools but no ChatGPT-triggered bash tool.", `${EDITION.cliName} start --root ${rootArg} --no-bash`),
    copyCommand("Require explicit bash target", "Restart so bash calls must include this matching session_id.", `${EDITION.cliName} start --root ${rootArg} --bash-session ${sessionArg} --require-bash-session`),
    copyCommand("Show Codex session list", "Restart with read-only local Codex session metadata in full tool mode.", `${EDITION.cliName} start --root ${rootArg} --tool-mode full --codex-sessions metadata`),
    copyCommand("Read Codex transcripts", "Restart with bounded local transcript reads from Codex JSONL history.", `${EDITION.cliName} start --root ${rootArg} --tool-mode full --codex-sessions read`),
    copyCommand("Use full bash transcript", "Restart with the raw stdout/stderr transcript instead of compact tool cards.", `${EDITION.cliName} start --root ${rootArg} --bash-transcript full`)
  ].join("");
  return `<!doctype html>
<html lang="en">
<head>
  ...
  <title>${EDITION.productName} Local Control - Personal Workspace Agent</title>
  <style>
...
<body>
  <main>
    <header class="topbar">
      <div class="brand">
        ...
        <span>
          ...
          <span class="brand-title">${EDITION.productName}</span>
        </span>
      </div>
      <nav class="quick-links" aria-label="${EDITION.productName} resources">
      ...
      <aside class="side-stack">
        <section class="panel guide-panel" id="guide">
          ...
          <div class="guide-list">
            ...
            <div class="guide-item"><span class="num">3</span><span><strong>Create the ChatGPT app</strong><p>${escapeHtml(chatgptAuthCopy(config))}</p></span></div>
            <div class="guide-item"><span class="num">4</span><span><strong>Restart for policy changes</strong><p>Saved profile changes apply when ${EDITION.productName} starts again. The live server does not mutate under an active ChatGPT session.</p></span></div>
          </div>
        </section>
        <article class="run-card" id="status" aria-label="Current runtime">
          ...
          <div class="status">
            ...
            <div class="row"><span class="label">Auth</span><span class="pill">${escapeHtml(authModeLabel)}</span></div>
          </div>
        </article>
      </aside>
    </section>
    <section class="workspace-grid">
      <section class="panel" id="connect">
        ...
        <ol class="steps">
          ...
          <li><span class="num">3</span><span>Paste the public ${EDITION.productName} URL from the terminal.</span></li>
          <li><span class="num">4</span><span>${escapeHtml(chatgptAuthCopy(config))}</span></li>
        </ol>
    ...
    <details class="panel details-panel">
      ...
      <p class="note">${EDITION.productName} rejects workspace access outside these roots.</p>
    </details>
  ...
  <script>
    document.querySelectorAll("[data-copy], [data-copy-kind]").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        if (button.getAttribute("data-copy-kind") === "local-mcp") {
          value = button.getAttribute("data-copy-base") || value;
    ...
    function serverPreviewFor(hostname) {
      ...
      if (!clean) return "";
      return "https://" + clean + "/mcp" + (tokenEnabled ? " (Authorization: Bearer <redacted>)" : "");
    }
    ...
    if (profileForm) {
      profileForm.addEventListener("submit", async (event) => {
        ...
        try {
          const bearerToken = tokenEnabled ? (window.localStorage.getItem("personalEditionBearerToken") || window.prompt("Paste your bearer token for admin profile access") || "") : "";
          if (tokenEnabled && !bearerToken) throw new Error("Bearer token is required to save the profile.");
          if (bearerToken) window.localStorage.setItem("personalEditionBearerToken", bearerToken);
          ...
          if (status) status.textContent = "Saved. Restart the personal edition for these profile settings to apply.";
        } catch (error) {
...
async function main(): Promise<void> {
  ...
  if (config.requireHttpToken && !config.authToken) {
    throw new Error(
      "PERSONAL_HTTP_TOKEN is required for this HTTP binding. " +
        "Set PERSONAL_HTTP_TOKEN, use `h7ymcp start` to generate one, " +
        "or set PERSONAL_ALLOW_NO_HTTP_TOKEN=1 only for a trusted local-only setup."
    );
  }

  const app = express();
  app.set("trust proxy", true);
  const logRequests = editionEnvValue("LOG_REQUESTS") === "1";
  const oauthState = config.httpAuthMode === "oauth" ? new LocalOAuthState(config.defaultRoot) : undefined;
  ...
  app.use((req, res, next) => {
    ...
    res.on("finish", () => {
    console.error(`[${EDITION.productName}] ${req.method} ${req.path} -> ${res.statusCode} ${Date.now() - started}ms`);
    });
    next();
  });
  ...
  function oauthProviderFor(req: Request): LocalOAuthProvider {
    if (!oauthState) {
      throw new Error("OAuth is not enabled for this server.");
    }
    const origin = requestOrigin(req);
    if (origin.protocol !== "https:") {
      throw new Error("OAuth discovery requires an HTTPS origin. Use a public HTTPS tunnel or disable OAuth for local-only runs.");
    }
    return new LocalOAuthProvider(oauthState, {
      approvalBaseUrl: origin,
      authToken: config.authToken,
      manualApproval: config.oauthApprovalMode === "manual",
      oauthScopes: config.oauthScopes.length ? config.oauthScopes : [...DEFAULT_OAUTH_SCOPES],
      resourceServerUrl: new URL("/mcp", origin)
    });
  }

  async function handleOAuthRouter(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const origin = requestOrigin(req);
      const provider = oauthProviderFor(req);
      const router = mcpAuthRouter({
        provider,
        issuerUrl: origin,
        baseUrl: origin,
        resourceServerUrl: new URL("/mcp", origin),
        scopesSupported: config.oauthScopes.length ? config.oauthScopes : [...DEFAULT_OAUTH_SCOPES],
        resourceName: `${EDITION.productName} MCP`
      });
      router(req, res, next);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      res.status(400).json({ error: "oauth_unavailable", error_description: message });
    }
  }

  const oauthApprovalBody = express.urlencoded({ extended: false });
  app.post("/oauth/approve", oauthApprovalBody, async (req, res) => {
    try {
      const provider = oauthProviderFor(req);
      const requestId = typeof req.body?.request_id === "string" ? req.body.request_id : "";
      const approvalToken = typeof req.body?.approval_token === "string" ? req.body.approval_token : "";
      await provider.approve({ requestId, approvalToken }, res);
    } catch (error) {
      res.status(400).type("html").send(`<h1>OAuth approval failed</h1><p>${escapeHtml(error instanceof Error ? error.message : String(error))}</p>`);
    }
  });
  app.post("/oauth/deny", oauthApprovalBody, async (req, res) => {
    try {
      const provider = oauthProviderFor(req);
      const requestId = typeof req.body?.request_id === "string" ? req.body.request_id : "";
      await provider.deny(requestId, res);
    } catch (error) {
      res.status(400).type("html").send(`<h1>OAuth denial failed</h1><p>${escapeHtml(error instanceof Error ? error.message : String(error))}</p>`);
    }
  });
  app.use((req, res, next) => {
    if (config.httpAuthMode !== "oauth" || !isOAuthPublicPath(req.path)) {
      next();
      return;
    }
    void handleOAuthRouter(req, res, next);
  });
  ...
  app.use((req, res, next) => {
    ...
    if (
      ...
      (config.httpAuthMode === "oauth" && (req.path === "/mcp" || isOAuthPublicPath(req.path)))
    ) {
      next();
      return;
    }
  ...
  app.post("/admin/profile", adminRateLimit, adminBodyLimit, express.json({ limit: "32kb" }), (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        message: `Saved. Restart ${EDITION.productName} for these profile settings to apply.`
      });
    } catch (error) {
    ...
    config.httpAuthMode === "oauth"
      ? (req: Request, res: Response, next: NextFunction) => {
          try {
            const origin = requestOrigin(req);
            const provider = oauthProviderFor(req);
            const resourceServerUrl = new URL("/mcp", origin);
            const middleware = requireBearerAuth({
              verifier: provider,
              requiredScopes: config.oauthScopes.length ? config.oauthScopes : [...DEFAULT_OAUTH_SCOPES],
            ...
            res
              .status(400)
              .json({ error: "oauth_unavailable", error_description: error instanceof Error ? error.message : String(error) });
          }
        }
      : (_req: Request, _res: Response, next: NextFunction) => next();

  app.post("/mcp", oauthMcpAuth, express.json({ limit: "20mb" }), async (req, res) => {
    try {
      ...
      if (!res.headersSent) {
        res.status(500).json({
          jsonrpc: "2.0",
          error: { code: -32603, message: `Internal ${EDITION.productName} MCP error. Check the local terminal for details.` },
          id: null
        });
      }
    }
  ...
  app.get("/mcp", oauthMcpAuth, handleSessionRequest);
  app.delete("/mcp", oauthMcpAuth, handleSessionRequest);

  app.listen(config.port, config.host, () => {
    console.error(`[${EDITION.productName}] HTTP MCP listening on http://${config.host}:${config.port}/mcp`);
    console.error(`[${EDITION.productName}] defaultRoot=${config.defaultRoot}`);
    console.error(`[${EDITION.productName}] allowedRoots=${config.allowedRoots.join(", ")}`);
    console.error(`[${EDITION.productName}] bashMode=${config.bashMode}`);
    console.error(`[${EDITION.productName}] writeMode=${config.writeMode}`);
    console.error(`[${EDITION.productName}] widgetDomain=${config.widgetDomain}`);
  });
}
