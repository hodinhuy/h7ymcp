...
export interface WorkspaceProfile {
  ...
  oauthApproval?: "token" | "manual" | string;
  port?: string;
...
export function codexProHome(): string {
  const customHome = editionEnvValue("HOME");
...
function legacyProfileDir(): string {
  return path.join(legacyCodexProHome(), "profiles");
}
...
function legacyRuntimeDir(): string {
  return path.join(legacyCodexProHome(), "runtime");
}
...
export function readWorkspaceProfile(root: string): WorkspaceProfile {
  ...
  const legacyProfilePath = path.join(legacyProfileDir(), `${profileIdForRoot(root)}.json`);
  const chosenPath = fs.existsSync(profilePath) ? profilePath : fs.existsSync(legacyProfilePath) ? legacyProfilePath : "";
  ...
  return { ...typed, profilePath: chosenPath };
}
...
export function readRuntimeConnection(root: string): RuntimeConnection {
  ...
  const legacyRuntimePath = path.join(legacyRuntimeDir(), `${profileIdForRoot(root)}.json`);
  const chosenPath = fs.existsSync(runtimePath) ? runtimePath : fs.existsSync(legacyRuntimePath) ? legacyRuntimePath : "";
  if (!chosenPath) return {};
  const runtime = readJsonFile(chosenPath);
