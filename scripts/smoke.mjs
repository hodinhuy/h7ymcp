...
const client = new McpStdioClient('node', ['dist/stdio.js', '--root', tmp, '--allow-root', tmp, '--bash', 'safe', '--tool-mode', 'full', '--write', 'workspace'], {
  cwd: path.resolve('.'),
...
if (!instructions.includes('git status, git diff, ls, find, npm test') || instructions.includes('Do not use bash for git status')) {
  throw new Error(`safe-mode server instructions were not aligned with bash policy: ${instructions}`);
}
...
const defaultAgentClient = new McpStdioClient('node', ['dist/stdio.js', '--root', tmp, '--allow-root', tmp], {
  cwd: path.resolve('.'),
  env: {
    ...process.env,
    PERSONAL_ROOT: tmp,
    PERSONAL_ALLOWED_ROOTS: tmp,
    PERSONAL_MODE: 'agent',
    PERSONAL_WRITE_MODE: '',
    CODEXPRO_ROOT: tmp,
...
await defaultAgentClient.request('initialize', {
  protocolVersion: '2024-11-05',
  capabilities: {},
  clientInfo: { name: 'codexpro-default-agent-smoke', version: '0.1.0' }
});
defaultAgentClient.notify('notifications/initialized');
const defaultAgentTools = await defaultAgentClient.request('tools/list', {});
const defaultAgentToolNames = defaultAgentTools.tools.map((tool) => tool.name);
for (const expectedWriteTool of ['write', 'edit']) {
  if (!defaultAgentToolNames.includes(expectedWriteTool)) {
    throw new Error(`agent mode without --write should advertise ${expectedWriteTool}: ${defaultAgentToolNames.join(', ')}`);
  }
}
const defaultAgentConfig = await defaultAgentClient.request('tools/call', { name: 'server_config', arguments: {} });
if (defaultAgentConfig.structuredContent.writeMode !== 'workspace' || !defaultAgentConfig.structuredContent.registeredTools?.includes?.('write') || !defaultAgentConfig.structuredContent.registeredTools?.includes?.('edit')) {
  throw new Error(`agent mode without --write did not resolve to workspace: ${JSON.stringify(defaultAgentConfig.structuredContent)}`);
}
defaultAgentClient.close();
const toolCardUri = 'ui://widget/codexpro-tool-card-v9.html';
const toolsByName = new Map(tools.tools.map((tool) => [tool.name, tool]));
function securitySchemeTypes(name) {
...
if (!securitySchemeTypes('server_config').includes('noauth')) {
  throw new Error(`expected noauth tool metadata in stdio mode, got ${JSON.stringify(toolsByName.get('server_config'))}`);
}
const cardClient = new McpStdioClient('node', ['dist/stdio.js', '--root', tmp, '--allow-root', tmp, '--bash', 'safe', '--tool-mode', 'full', '--write', 'workspace'], {
  cwd: path.resolve('.'),
  env: {
    ...process.env,
    CODEXPRO_ROOT: tmp,
...
const cardResources = await cardClient.request('resources/list', {});
const cardToolCard = cardResources.resources.find((resource) => resource.uri === toolCardUri);
if (!cardToolCard) throw new Error(`missing opt-in tool-card resource: ${toolCardUri}`);
if (cardToolCard.mimeType !== 'text/html;profile=mcp-app') throw new Error(`unexpected tool-card mime type: ${cardToolCard.mimeType}`);
const legacyToolCardUri = 'ui://widget/codexpro-tool-card-v8.html';
const cardLegacyToolCard = cardResources.resources.find((resource) => resource.uri === legacyToolCardUri);
if (!cardLegacyToolCard) throw new Error(`missing legacy tool-card resource: ${legacyToolCardUri}`);
const widget = await cardClient.request('resources/read', { uri: toolCardUri });
...
const legacyWidget = await cardClient.request('resources/read', { uri: legacyToolCardUri });
...
try {
  ...
  if (toolCard) {
    throw new Error(`tool-card resource should not be registered while CODEXPRO_TOOL_CARDS is off: ${toolCardUri}`);
  }
} catch (error) {
...
const resolvedCurrentRoot = await fs.realpath(current.structuredContent.root);
if (resolvedCurrentRoot !== realTmp) throw new Error(`open_current_workspace opened ${current.structuredContent.root} (resolved ${resolvedCurrentRoot}), expected ${realTmp}`);
...
const gitStatusBash = await client.request('tools/call', { name: 'bash', arguments: { workspace_id: ws, command: 'git status --short', timeout_ms: 60000 } });
const gitStatusBashText = gitStatusBash.content?.[0]?.text ?? '';
if (!gitStatusBashText.includes('Exit: 0') || gitStatusBashText.includes('## stdout') || gitStatusBashText.includes('## stderr')) {
  throw new Error(`default bash transcript should be compact: ${gitStatusBashText}`);
}
if (!gitStatusBash.structuredContent.stdout?.includes('M demo.txt')) {
  throw new Error(`safe bash did not run git status --short: ${JSON.stringify(gitStatusBash.structuredContent)}`);
}
...
async function assertToolMode(mode, expected, hidden) {
  const args = ['dist/stdio.js', '--root', tmp, '--allow-root', tmp, '--bash', 'safe', '--write', 'workspace'];
...
if (invalidContextDir.status === 0 || !/PERSONAL_CONTEXT_DIR|CODEXPRO_CONTEXT_DIR/.test(String(invalidContextDir.stderr || invalidContextDir.stdout))) {
