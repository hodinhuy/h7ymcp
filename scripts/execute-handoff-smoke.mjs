...
function gitCommit(cwd, label) {
  requireSuccess(
    spawnSync('git', ['-c', 'user.name=Smoke Test', '-c', 'user.email=smoke@example.com', 'commit', '-m', 'initial'], {
      cwd,
      encoding: 'utf8'
    }),
    label
...
const planPath = process.argv[taskIndex + 1];
const plan = fs.readFileSync(planPath, 'utf8');
const model = modelIndex >= 0 ? process.argv[modelIndex + 1] : '';
const appPath = path.join(path.dirname(planPath), '..', 'app.txt');
fs.appendFileSync(appPath, \`implemented with \${model}: \${plan.includes('implementation marker') ? 'yes' : 'no'}\\n\`);
...
gitCommit(root, 'git commit initial');
...
const plan = fs.readFileSync(planPath, 'utf8');
const appPath = path.join(path.dirname(planPath), '..', 'app.txt');
fs.appendFileSync(appPath, \`watch implemented: \${plan.split('\\n')[0]}\\n\`);
...
gitCommit(watchRoot, 'watch git commit initial');
...
const appPath = path.join(path.dirname(planPath), '..', 'app.txt');
fs.appendFileSync(appPath, \`loop \${marker}\\n\`);
...
gitCommit(loopRoot, 'loop git commit initial');
...
await fs.writeFile(path.join(subdirUntrackedRoot, 'agent.mjs'), `process.exit(0);\n`, 'utf8');
await fs.writeFile(path.join(subdirUntrackedRoot, 'reviewer.mjs'), `console.log('CODEXPRO_REVIEW=PASS');\n`, 'utf8');
